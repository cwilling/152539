/* 
 * process.COBN.c is derived from:
 * @file code_COBN.c
 ...
 * @date 2 2014
 * downloaded August 2025 from https://github.com/ModelDBRepository/152539
 *
 * process.COBN.c assumes:
 * array A has been generated elsewhere is passed here as an input parameter.
 *
 * Array Ncon is recalculated in case of change to A.
*/
        ...
        
{
	...
	*/
	...
	// Inputs should be: net_COBN, INPUT2E, INPUT2I, tLastSP, aX_rec, aX_ext, gX, aS_rec, aS_ext, gS, V, A, E2EI, I2EI, eFR, iFR, eSPC, iSPC
    if (nrhs != 18)
    ...
    */
    ...
    /* Which time step to (re)start processing */
    tmp   = mxGetField(prhs[0], 0, "restart" );
    restart = *mxGetPr(tmp);

    /* How many time steps to process */
    ...
    // Printing the network's parameters only on first iteration
    if (restart == 0)
    {
    ...
    {   // Use recyled versions 
        mxArray *E2EIcopy = mxDuplicateArray(prhs[12]);
        plhs[0] = E2EIcopy;
        e2eI = mxGetPr(plhs[0]);

        mxArray *I2EIcopy = mxDuplicateArray(prhs[13]);
        plhs[1] = I2EIcopy;
        i2eI = mxGetPr(plhs[1]);

        mxArray *eFRcopy = mxDuplicateArray(prhs[14]);
        plhs[2] = eFRcopy;
        eFR = (unsigned short*)mxGetPr(plhs[2]);

        mxArray *iFRcopy = mxDuplicateArray(prhs[15]);
        plhs[3] = iFRcopy;
        iFR = (unsigned short*)mxGetPr(plhs[3]);
    }
    ...
    else
    {
        ...
        mxArray *aX_recCopy = mxDuplicateArray(prhs[4]);
        plhs[5] = aX_recCopy;
        aX_rec = mxGetPr(plhs[5]);

        mxArray *aX_extCopy = mxDuplicateArray(prhs[5]);
        plhs[6] = aX_extCopy;
        aX_ext = mxGetPr(plhs[6]);

        mxArray *gXcopy = mxDuplicateArray(prhs[6]);
        plhs[7] = gXcopy;
        gX = mxGetPr(plhs[7]);

        mxArray *aS_recCopy = mxDuplicateArray(prhs[7]);
        plhs[8] = aS_recCopy;
        aS_rec = mxGetPr(plhs[8]);

        mxArray *aS_extCopy = mxDuplicateArray(prhs[8]);
        plhs[9] = aS_extCopy;
        aS_ext = mxGetPr(plhs[9]);

        mxArray *gScopy = mxDuplicateArray(prhs[9]);
        plhs[10] = gScopy;
        gS = mxGetPr(plhs[10]);

        mxArray *Vcopy = mxDuplicateArray(prhs[10]);
        plhs[11] = Vcopy;
        V = mxGetPr(plhs[11]);
    }
    
    /* Ncon: array with the number of outgoing connections of each neuron.
     * Generate a new Ncon based on a possibly changed A (received as input parameter).
    */
    ...
    {
        mxArray *eSPCcopy = mxDuplicateArray(prhs[16]);
        plhs[12] = eSPCcopy;
        eSPC = mxGetPr(plhs[12]);

        mxArray *iSPCcopy = mxDuplicateArray(prhs[17]);
        plhs[13] = iSPCcopy;
        iSPC = mxGetPr(plhs[13]);
    }

    /* A: array with the connections of each neuron.
    *  Always use provided version which shouldnn't change so don't bother recycling.
    */
    A = (mwSize*)mxGetPr(prhs[11]);

	// Construct Ncon from (possibly new) A
    for(nrn=0; nrn<totNnrn; nrn++)
    {
        for(nrn2=0; nrn2<totNnrn; nrn2++)
        {
            // Special case: value of first entry (nrn2 == 0) can validly be 0
            // (since we're counting connections to neurons from 0 to Ntot-1).
            if (nrn2 == 0)
            {
                if (A[nrn*totNnrn + nrn2] >= 0 )
                    Ncon[nrn]++;
            }
            else
            {
    ...
    // else we add to the recycled tLastSP, V passed as input parameters
        ...
        */   
        ...
        for(nrn=0; nrn<totNnrn; nrn++) {
            ...
            */
            ...
            /* GABA synapses: ----------------------------------------------------*/
            if(nrn<eNnrn){
				// Should this be if (nrn<iNnrn) ?? (not <eNnrn)
    ...
    if (restart == 0)
    {
        plhs[4] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *tLastSPcopy = mxGetPr(plhs[4]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            tLastSPcopy[nrn] = tLastSP[nrn];
        }
        mxFree(tLastSP);

        plhs[5] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *aX_recCopy = mxGetPr(plhs[5]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            aX_recCopy[nrn] = aX_rec[nrn];
        }
        mxFree(aX_rec);

        plhs[6] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *aX_extCopy = mxGetPr(plhs[6]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            aX_extCopy[nrn] = aX_ext[nrn];
        }
        mxFree(aX_ext);

        plhs[7] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *gXcopy = mxGetPr(plhs[7]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            gXcopy[nrn] = gX[nrn];
        }
        ...
        plhs[8] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *aS_recCopy = mxGetPr(plhs[8]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            aS_recCopy[nrn] = aS_rec[nrn];
        }
        mxFree(aS_rec);

        plhs[9] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *aS_extCopy = mxGetPr(plhs[9]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            aS_extCopy[nrn] = aS_ext[nrn];
        }
        mxFree(aS_ext);

        plhs[10] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *gScopy = mxGetPr(plhs[10]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            gScopy[nrn] = gS[nrn];
        }
        ...
        plhs[11] = mxCreateDoubleMatrix(totNnrn, 1, mxREAL);;
        double *Vcopy = mxGetPr(plhs[11]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            Vcopy[nrn] = V[nrn];
        }
        ...
        plhs[12] = mxCreateDoubleMatrix(totNnrn * eCycSize, 1, mxREAL);;
        double *eSPCcopy = mxGetPr(plhs[12]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            eSPCcopy[nrn] = eSPC[nrn];
        }
        mxFree(eSPC);

        plhs[13] = mxCreateDoubleMatrix(totNnrn * iCycSize, 1, mxREAL);;
        double *iSPCcopy = mxGetPr(plhs[13]);
        for(nrn=0; nrn<totNnrn; nrn++)
        {
            iSPCcopy[nrn] = iSPC[nrn];
        }
        mxFree(iSPC);
    }
    // otherwise tLastSP & V are recycled and updated
    // also aX_rec, aX_ext, gX, aS_rec, aS_ext, gS
    // also eSPC & iSPC
